export default definePreset({
  ...
  handler: async (context) => {
    await prompt({
      name: 'fileName',
      text: '*Required*: What is the name of the endpoint? (eg: ShowUser)',
      default: null
    });
    await prompt({
      name: 'subfolder',
      text: '*Required*: What subfolder will the endpoint reside in? (eg: Users)',
      default: ''
    });
    await prompt({
      name: 'domain',
      text: 'What domain does the endpoint belong to? Leave blank if not using domains. (eg: Auth)',
      default: ''
    });
